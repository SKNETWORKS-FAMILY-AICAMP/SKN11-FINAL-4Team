...
3. 친근하고 자연스러운 대화를 유지하세요"""
    ...
    def get_random_tone_descriptions(self, character: CharacterProfile) -> dict:
        ...
        for i, prompt in enumerate(prompts):
            response = self.client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[{"role": "system", "content": "아래 프롬프트에 따라 어조 스타일 설명을 한 문장으로, 반드시 한국어로만 답변하세요."},
    ...
    def generate_character_random_tones_sync(self, messages: List[str], character: CharacterProfile) -> Dict[str, Any]:
        """
