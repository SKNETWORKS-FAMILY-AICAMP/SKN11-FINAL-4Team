...
async def chatbot_stream(
    ...
    async def generate_stream():
        try:
            ...
            # 시작 메시지
            yield f"data: {json.dumps({'type': 'start', 'session_id': session_id, 'influencer_name': str(influencer.influencer_name)})}\n\n"
            
            # 응답을 단어 단위로 스트리밍
            words = response_text.split()
            for i, word in enumerate(words):
                yield f"data: {json.dumps({'type': 'token', 'content': word + ' ', 'index': i})}\n\n"
                await asyncio.sleep(0.1)  # 100ms 딜레이
            
            # 완료 메시지
            yield f"data: {json.dumps({'type': 'end', 'full_response': response_text})}\n\n"
