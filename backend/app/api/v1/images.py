...
async def generate_image(
    ...
    try:
        logger.info(f"Image generation request from user {current_user.get('sub')}: {request.prompt[:50]}...")
        ...
        raise HTTPException(status_code=500, detail=f"이미지 생성에 실패했습니다: {str(e)}")
...
async def upload_image(
    ...
    try:
        ...
        logger.info(f"Image uploaded by user {current_user.get('sub')}: {file.filename}")
...
async def serve_image(image_path: str):
    ...
    try:
        ...
        if hasattr(storage_service, 'storage_path'):
            file_path = storage_service.storage_path / image_path
            ...
            # 보안: 저장소 경로 외부 접근 방지
            try:
                file_path.resolve().relative_to(storage_service.storage_path.resolve())
            except ValueError:
...
async def delete_image(
    ...
    try:
        ...
        if current_user.get('sub') not in image_path:
        ...
        if success:
            logger.info(f"Image deleted by user {current_user.get('sub')}: {image_path}")
...
async def get_image_info(
    ...
    try:
        ...
        if image_info:
            ...
            raise HTTPException(status_code=404, detail="이미지 정보를 찾을 수 없습니다.")
