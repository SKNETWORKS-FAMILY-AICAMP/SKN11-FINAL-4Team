...
from app.services.influencers.qa_generator import InfluencerQAGenerator, QAGenerationStatus
...
class QAGenerationService:
    ...
    async def generate_qa_for_influencer(
        ...
        if num_qa_pairs < 1 or num_qa_pairs > 5000:
            raise HTTPException(status_code=400, detail="QA 쌍 개수는 1-5000 사이여야 합니다")
        
        if len(domains) == 0:
            raise HTTPException(status_code=400, detail="최소 1개 이상의 도메인을 선택해야 합니다")
        ...
        try:
            ...
            try:
                ...
                task_id = qa_service.qa_generator.start_qa_generation(influencer_id, db, user_id)
                
                logger.info(f"✅ QA 생성 작업 시작 완료: task_id={task_id}, 도메인={domains}")
            ...
            raise HTTPException(
                status_code=500, 
                detail=f"QA 생성 중 오류가 발생했습니다: {str(e)}"
            )
        ...
        try:
            ...
            try:
                ...
                if batch_key_entry.s3_processed_file_url:
                    s3_urls["processed_file_url"] = str(batch_key_entry.s3_processed_file_url)
                
                # 진행률 계산
                progress = 0
                if batch_key_entry.total_qa_pairs and batch_key_entry.generated_qa_pairs:
                    progress = (batch_key_entry.generated_qa_pairs / batch_key_entry.total_qa_pairs) * 100
            ...
            raise HTTPException(
                status_code=500, 
                detail=f"QA 상태 조회 중 오류가 발생했습니다: {str(e)}"
            )
    ...
    async def cancel_qa_generation(task_id: str, user_id: str) -> dict:
        ...
        try:
            ...
            try:
                ...
                batch_key_entry.updated_at = datetime.now()
            ...
            raise HTTPException(
                status_code=500,
                detail=f"QA 작업 취소 중 오류가 발생했습니다: {str(e)}"
            )


# 하위 호환성을 위한 개별 함수
async def generate_qa_for_influencer(influencer_id: str, user_id: str, **kwargs) -> dict:
    """QA 생성 (하위 호환성)"""
    return await QAGenerationService.generate_qa_for_influencer(influencer_id, user_id, **kwargs)
