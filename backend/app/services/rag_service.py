    ...
    async def search_similar(self, query: str, top_k: int = 5) -> List[Dict]:
        ...
        try:
            ...
            for item in similarities[:top_k]:
                if item["similarity"] >= self.config.score_threshold:
                    results.append({
...
class RAGService:
    ...
    async def chat(self, group_id: int, query: str, 
        ...
        try:
            ...
            if include_sources and search_results:
                ...
                result["context_preview"] = context[:200] + "..." if len(context) > 200 else context
    ...
    def list_pipelines(self) -> List[Dict]:
        """모든 파이프라인 목록"""
        return [
            {"group_id": group_id, **info}
            for group_id, info in self._pipelines.items()
        ]
