...
class MCPClientService:
    ...
    async def initialize_mcp_client(self):
        """langchain-mcp-adapters를 사용한 MCP 클라이언트 초기화"""
        try:
            ...
            for server_name, config in server_configs.items():
                try:
                    ...
                    logger.warning(f"❌ MCP 서버 '{server_name}' 설정 구성 실패: {e}")
            ...
            logger.info(f"🔄 MultiServerMCPClient 초기화 중... (설정: {client_config})")
            ...
            for server_name in server_configs.keys():
                ...
                try:
                    async with self.mcp_client.session(server_name) as session:
                        ...
                        logger.info(
                            f"✅ MCP 서버 '{server_name}' 도구 로드 완료: {len(tools)}개"
                        )
            ...
            logger.info(
                f"🎯 MCP 클라이언트 초기화 완료: 총 {len(self.mcp_tools)}개 서버, {total_tools}개 도구"
            )
    ...
    async def get_tools(self, server_name: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if server_name in self.mcp_tools:
                ...
                logger.warning(f"MCP 서버 '{server_name}'에서 도구를 찾을 수 없습니다")
                return []
    ...
    async def execute_tool(
        ...
        try:
            ...
            # 항상 새로운 세션을 열고 그 안에서 도구를 찾아 실행
            try:
                logger.info(f"🔄 서버 '{server_name}'에 새 세션 열기...")
                async with self.mcp_client.session(server_name) as session:
                    # 세션을 통해 도구 로드
                    from langchain_mcp_adapters.tools import load_mcp_tools

                    tools = await load_mcp_tools(session)
                    logger.info(f"  - 세션에서 로드된 도구 수: {len(tools)}")
