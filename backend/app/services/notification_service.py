...
class EmailService:
    def __init__(self):
        ...
        if not self.is_configured:
            logger.warning("이메일 서비스 설정이 완전하지 않습니다. 환경변수를 확인해주세요.")
    
    def create_finetuning_completion_template(self, influencer_name: str, model_url: str) -> NotificationTemplate:
        """파인튜닝 완료 이메일 템플릿 생성"""
...
class WebNotificationService:
    ...
    def create_notification(self, user_id: str, title: str, message: str, 
                          notification_type: str = "info") -> WebNotification:
        """웹 알림 생성"""
    ...
    def get_user_notifications(self, user_id: str, unread_only: bool = False) -> List[WebNotification]:
        """사용자 알림 조회"""
...
class NotificationService:
    ...
    async def send_finetuning_completion_notification(self, user_email: str, user_id: str,
                                                    influencer_name: str, model_url: str):
        """파인튜닝 완료 알림 전송 (이메일 + 웹)"""
        try:
            # 이메일 알림
    ...
    def get_web_notifications(self, user_id: str, unread_only: bool = False) -> List[Dict]:
        """웹 알림 조회 (API 응답용)"""
