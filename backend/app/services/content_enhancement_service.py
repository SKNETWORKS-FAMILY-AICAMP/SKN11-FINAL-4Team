...
class ContentEnhancementService:
    ...
    async def _call_openai_enhancement(
        ...
        if board_platform is not None:
            ...
            platform_char = platform_characteristics.get(board_platform, "소셜미디어 게시글에 적합하게")
...
"""
    ...
    def get_user_enhancements(
        ...
        enhancements = db.query(ContentEnhancement).filter(
            ContentEnhancement.user_id == user_id
        ).order_by(ContentEnhancement.created_at.desc()).offset(offset).limit(page_size).all()
        
        total_count = db.query(ContentEnhancement).filter(
            ContentEnhancement.user_id == user_id
        ).count()
