...
class ComfyUIService:
    ...
    def __init__(self, comfyui_server_url: str = None):
        ...
        # ComfyUI 서버 연결 확인
        self.use_mock = not self._check_server_connection()
    ...
    async def _wait_for_completion(self, session: aiohttp.ClientSession, job_id: str) -> List[str]:
            ...
            try:
                async with session.get(
                    ...
                    if response.status == 200:
                        ...
                        if job_id in history and history[job_id].get("status", {}).get("completed", False):
                            # 생성 완료, 이미지 수집
    ...
    async def _download_image(self, session: aiohttp.ClientSession, filename: str, subfolder: str, type_: str) -> Optional[str]:
