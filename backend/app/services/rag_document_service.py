...
class DocumentProcessor:
    ...
    async def process_pdf(self, pdf_path: str) -> List[Dict]:
        """PDF 문서 처리"""
        if not PdfReader:
            raise ImportError("PyPDF2가 설치되지 않았습니다. pip install PyPDF2를 실행하세요.")
    ...
    async def _generate_qa_pairs(self, chunks: List[str], source_file: str) -> List[Dict]:
        """청크에서 QA 쌍 생성"""
        qa_pairs = []
...
class AdvancedDocumentProcessor(DocumentProcessor):
    ...
    async def _generate_qa_with_ai(self, chunks: List[str], source_file: str) -> List[Dict]:
        """AI를 사용한 QA 쌍 생성"""
