...
### 현재 상태
- **RunPod 서비스**: 실제 API 키 요구, Mock 모드 없음
- **워크플로우**: `custom_workflow` → `basic_txt2img` 자동 폴백
- **오류 처리**: 실제 서비스 장애 시 명확한 예외 발생
- **환경 설정**: `.env` 파일에 실제 API 키 설정 필요

### 다음 단계
1. 실제 RunPod API 키 및 ComfyUI 템플릿 ID 확인
2. 전체 이미지 생성 프로세스 테스트
3. Pod 생성 → 워크플로우 전송 → 이미지 수신 → Pod 종료 플로우 검증
