...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, inference_params: InferenceParams, freqs_cis: torch.Tensor) -> torch.Tensor:
        ...
        # GQA를 위한 key/value 헤드 확장
        if self.num_heads_kv != self.num_heads:
            # key와 value를 query 헤드 수에 맞게 반복
            k = k.repeat_interleave(self.num_heads // self.num_heads_kv, dim=1)
            v = v.repeat_interleave(self.num_heads // self.num_heads_kv, dim=1)

        # enable_gqa는 PyTorch 2.1+ 에서만 지원되므로 제거
